## Adapted from StyleGAN2-ADA &mdash; Official PyTorch implementation

Folder "scripts" contain various scripts used in the AAPM DGM Challenge:
1. collate.sh - given a parent dir moves all contents of the sub-dirs into the parent dir.
2. downsize_with_antialiasing.py - downsizes the ressolution of images with antialiasing (for proper resizing per https://github.com/GaParmar/clean-fid)
3. eval_script.swb - runs KS_stat_metric.py for each subdir (containing a set of images) held in a parent dir, n number of times.
4. generate.swb - generates fake images given a neural network (StyelGAN2)
5. KS_stat_metric - custom metric adapted from AAPM DGM challenge (public eval code) for evaluation of fakes per medically significant statistics
6. swagan-exp-1.swb - train the swagan network from a different repo (https://github.com/rosinality/stylegan2-pytorch)
7. train_exp.swb - Train StyleGAN2 network (from this repo)

### ---Begin original readme file from https://github.com/NVlabs/stylegan2-ada-pytorch ---
